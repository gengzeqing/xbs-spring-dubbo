package com.xuebusi.xbs.util.lock;
...
public class DistributedLock {
	 ...
	 * @param leaseTime:<p>加锁时间,默认支持过期解锁功能,30秒钟以后自动解锁,无需调用unlock方法手动解锁</p>
	...
	public boolean lock(String key, Integer leaseTime) throws CacheException {
		...
		if (leaseTime == null) {
			leaseTime = DistributedConstat.REDI_SIMPLE_AUTO_TIME; // [走系统默认,最多等待30秒，上锁以后30秒自动解锁]
		}
		LogMgr.sysInfo("do begin.. lock->",key);
		RLock redissonLock = redissonManager.getRedisson().getLock(lockKey);
		LogMgr.sysInfo("do begin ### get RLock->",redissonLock);
		try {
			redissonLock.lock(leaseTime, TimeUnit.SECONDS);// [支持过期解锁功能,30秒钟以后自动解锁,无需调用unlock方法手动解锁]
			LogMgr.sysInfo("threadName:", Thread.currentThread().getName(),DistributedConstat.LOG_PREFIX, "LockKey:", lockKey," is --> [LOCK]");
...
//			}
	 ...
	 * @throws CacheException
	 */
	public boolean tryLock(String key,long waitTime, long leaseTime) throws CacheException {
		String lockKey = generateKey(DistributedConstat.LOCK_SUFFIX, key); //
		if (RedisKeyRule.isKeyRule(key) == Boolean.FALSE) {
			throw new CacheException("distributed to Lock is error,this key not effective,key value-> [" + key+ "],redis key be similar to [object-model-key]");
		}
		...
		LogMgr.sysInfo("do begin ### get RLock->",redissonLock);
		 ...
		 try {
			boolean lockStatus = redissonLock.tryLock(waitTime, leaseTime, TimeUnit.SECONDS);
			return lockStatus;
...
//			}
	...
	public void unLock(String key) throws CacheException {
		...
		redissonLock.unlockAsync(); // 通知当前线程释放锁 -> 清理资源时删除锁。此部分比较简单粗暴，删除锁成功则并发布锁被删除的消息，返回1结束，否则返回0结束。
		this.unlockInnerAsync(lockKey);
//		redissonLock.unlock();
	 ...
	 * @param lockKey
	 */
	private void unlockInnerAsync(String lockKey) {
		RLock redissonLock = redissonManager.getRedisson().getLock(lockKey);
		if (redissonLock.isLocked() == Boolean.TRUE) { 
			redissonLock.unlock(); // Boolean opStatus = get(unlockInnerAsync(Thread.currentThread().getId()));
			/*
			 * exists then publish | del  -> publish
			 * 执行顺序
			 */
		}
	}
